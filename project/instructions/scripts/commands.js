...
document.addEventListener('DOMContentLoaded', function() {
    initExpandableCommandLists();
    initCommandCardAnimations();
    initStatsCounter();
    initOrbitalSystem();
...
// ==========================================
function initExpandableCommandLists() {
    ...
    expandButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const commandsGrid = categoryBlock.querySelector('.commands-grid');
            const arrow = this.querySelector('.arrow');
            const buttonText = this.querySelector('span:first-child');

            // Toggle expanded state
            const isExpanded = commandsGrid.classList.contains('expanded');

            if (isExpanded) {
                // Collapse
                commandsGrid.classList.remove('expanded');
                arrow.textContent = '▼';
                buttonText.textContent = '展开查看';
                this.classList.remove('active');
            } else {
                // Expand
                commandsGrid.classList.add('expanded');
                arrow.textContent = '▲';
                buttonText.textContent = '收起列表';
                this.classList.add('active');

                // Animate command cards
                animateCommandCards(commandsGrid);
            }
        });
    });
}

function animateCommandCards(commandsGrid) {
...
// ==========================================
// Command Card Scroll Animations
// ==========================================
function initCommandCardAnimations() {
...
function initOrbitalSystem() {
    const categoryNodes = document.querySelectorAll('.command-category-node');
    ...
    categoryNodes.forEach(node => {
        ...
        node.addEventListener('click', function() {
            const category = this.classList.contains('node-learning') ? 'learning' :
                           this.classList.contains('node-execution') ? 'execution' :
                           'project';
...
function initKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        if (sectionMap[e.key]) {
            const section = document.querySelector(`.${sectionMap[e.key]}-section, .${sectionMap[e.key]}-category`);
            if (section) {
...
// ==========================================
function initWorkflowStepsAnimation() {
    const flowSteps = document.querySelectorAll('.flow-step');
    const observerOptions = {
        threshold: 0.3
    };
...
initWorkflowStepsAnimation();

// ==========================================
// Command Card Badge Hover Effects
// ==========================================
function initBadgeHoverEffects() {
    ...
    commandCards.forEach(card => {
        card.addEventListener('mouseenter', function() {
            const shortcut = this.querySelector('.command-shortcut');
            if (shortcut) {
                shortcut.style.transform = 'scale(1.1)';
            }
        });
...
function initParallaxEffect() {
    ...
    const orbitalSystem = document.querySelector('.commands-orbital-system');
...
console.log('%c' +
    '╔═══════════════════════════════════════╗\n' +
    '║  ZTL数智化作战中心 - Commands System ║\n' +
    '║  24 Commands | 3 Categories          ║\n' +
    '╚═══════════════════════════════════════╝',
