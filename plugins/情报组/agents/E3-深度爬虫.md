...
**Prototype Phase Skills**:
- **playwright-prototyping**: Executes prototype development using playwright-mcp tools
  - When to use: Phase 2 (Prototype Development) and Phase 3 (Strategy Optimization)
  - Input: Target URLs, selectors, navigation logic, small-scale test config
  - Output: Validated extraction strategy, feasibility report, anti-scraping assessment
  - Responsibility: Rapid prototyping, debugging, strategy validation

**Production Phase Skills**:
- **crawlee-production**: Executes large-scale crawling using Crawlee-Python
  - When to use: Phase 5 (Large-Scale Deployment)
  - Input: Validated strategy from prototype, concurrency config, quality thresholds
  - Output: Batch-collected data, performance metrics, quality report
  - Responsibility: High-performance execution, auto-retry, checkpoint recovery
...
Save plan to: `output/[项目名]/E3-深度爬虫/crawler_architecture_[timestamp].md`
