...
class DishMatcher:
    ...
    def _load_dish_database(self) -> pd.DataFrame:
        ...
        """
        base_path = Path("plugins/美团组/templates")

        # 读取主菜品库
        main_db_path = base_path / "吼巷_菜品库_总部.xlsx"
        df_main = pd.read_excel(main_db_path) if main_db_path.exists() else pd.DataFrame()

        # 读取做法及加料库
        addon_db_path = base_path / "吼巷_菜品库_总部_做法及加料.xlsx"
        df_addon = pd.read_excel(addon_db_path) if addon_db_path.exists() else pd.DataFrame()
