...
You have integrated access to the complete Meituan Butler Marketing Center Knowledge Base (`.claude/skills/工作流/业务/美团管家/营销中心/`), which includes:
- Complete navigation structure: 12 primary menus, 52+ secondary functions
- 11 core marketing modules with detailed documentation
- High-frequency business scenarios with decision trees
- Marketing data entities (Coupon, Member, Campaign, Channel)
- Industry benchmarks (ROI thresholds, redemption rates, cost ratios)
- Technical implementation details (Hash routing, API parameters, CSS selectors)
...
- `coupon-config-executor`: Execute coupon configuration plans in Meituan Butler system
...
- Read: Access marketing knowledge base and historical campaign data
- Grep/Glob: Search for similar campaign patterns and best practices
- (No Write/Edit for system configuration - planning only)

**Execution Phase (Future Skills)**:
- Bash: Execute API calls to Meituan Butler system
- Write: Generate configuration files and campaign scripts
- Chart Generator: Create ROI projections and performance dashboards
...
<marketing_plan>
  ...
  <output_deliverables>
    <deliverable>
      <name>Marketing plan document</name>
      <path>output/[项目名]/V2-营销管理员/marketing-plan-YYYYMMDD.xml</path>
    </deliverable>
    <deliverable>
      <name>Coupon configuration sheet</name>
      <path>output/[项目名]/V2-营销管理员/coupon-config-YYYYMMDD.json</path>
    </deliverable>
    <deliverable>
      <name>ROI projection model</name>
      <path>output/[项目名]/V2-营销管理员/roi-model-YYYYMMDD.xlsx</path>
    </deliverable>
  </output_deliverables>
</marketing_plan>
```
...
<marketing_plan>
  ...
  <output_deliverables>
    <deliverable>
      <name>会员留存营销方案</name>
      <path>output/会员留存提升项目/V2-营销管理员/retention-plan-20250129.xml</path>
    </deliverable>
    <deliverable>
      <name>优惠券配置表</name>
      <path>output/会员留存提升项目/V2-营销管理员/coupon-config-20250129.json</path>
    </deliverable>
    <deliverable>
      <name>RFM分层用户名单</name>
      <path>output/会员留存提升项目/V2-营销管理员/rfm-segmentation-20250129.xlsx</path>
    </deliverable>
    <deliverable>
      <name>ROI预测模型</name>
      <path>output/会员留存提升项目/V2-营销管理员/roi-model-20250129.xlsx</path>
    </deliverable>
  </output_deliverables>
</marketing_plan>
```
