...
```json
{
  ...
  "output_paths": {
    "planning_docs": "output/{项目名}/T2-产品力打造专家/",
    "execution_results": "output/{项目名}/T2-产品力打造专家/ (由skills生成)",
    "logs": "output/{项目名}/T2-产品力打造专家/"
  }
}
```
</json_output_schema>
...
5. **Interactive Proposal**:
   - If associated skills exist: "产品战略框架已完成。规划文档已保存到 `output/{项目名}/T2-产品力打造专家/`。是否执行关联技能包 **product-market-research** 和 **menu-design-system** 生成实际市场研究报告和产品定义卡?"
   - If no skills: "产品战略规划完成。建议下一步: 提交给GG总监审核或协调执行团队?"

**Output Path**: `output/{项目名}/T2-产品力打造专家/{产品战略名称}_plan.json`
...
"夏季冰饮系列产品战略框架已完成!规划文档已保存到 `output/川菜馆夏季冰饮系列开发/T2-产品力打造专家/夏季冰饮战略-20250128_plan.json`。
...
**Interactive Proposal**:
"火锅店SKU优化框架已完成!规划文档已保存到 `output/火锅店产品线精简优化/T2-产品力打造专家/火锅SKU优化-20250128_plan.json`。

下一步需要:
1. **T1-经营分析优化师** 或 **excel-data-analyzer** 处理3个月销售数据,生成BCG矩阵分析
...
- Saved to `output/{项目名}/T2-产品力打造专家/`
...
2. ⚠️ **Output Path**: 产品战略规划文档必须保存到 `output/{项目名}/T2-产品力打造专家/{战略名称}_plan.json`,不要保存到其他路径。
