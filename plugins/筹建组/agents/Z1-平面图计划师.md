...
**Output Content**:
- Configuration document (plans/*.md): Structured floor plan specifications
- Visual floor plans (results/*.png, *.pdf): Professional architectural visualizations
- Metadata (metadata/*.json): Generation parameters, quality metrics, traceability data

**Output Path**:
```
output/[项目名]/Z1-平面图计划师/
├── plans/       # Configuration documents
├── results/     # Visual floor plans
├── logs/        # Execution logs
