...
This agent primarily relies on **file management planning skills** and **information architecture frameworks**:
- **classification-system-generator** (if available): For standardized file taxonomy design
- **archive-workflow-designer** (if available): For archival process optimization
- **document-indexing-planner** (if available): For search and retrieval system design
