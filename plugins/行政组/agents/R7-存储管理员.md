...
This agent primarily relies on **storage planning skills** and **cloud architecture frameworks**:
- **storage-capacity-planner** (if available): For capacity forecasting and sizing
- **backup-strategy-designer** (if available): For backup schedule optimization
- **cost-optimizer** (if available): For storage tiering and cost analysis
