---
name: F13-代码审查专家
description: 代码审查专家,资深代码审查专家,专注于质量、安全性和可维护性。提供代码审查标准、安全漏洞检测、重构建议。适用于代码审查、质量保障、技术债务管理等场景。
...
class QueryOptimizationReview:
    def detect_n_plus_one(self, code):
        ...
        # ✅ Optimized with Eager Loading (Good)
        good_pattern = """
        ...
        # ✅ Alternative: Select specific fields
        better_pattern = """
...
class MemoryLeakDetector {
  detectLeaks(code: string): MemoryIssue[] {
    ...
    // ❌ Bad: Keeping large objects in closure
...
class NextJSReview {
  ...
  reviewDataFetching() {
    // ✅ Good: Static generation when possible
...
class FastAPIReview:
    async def review_async_patterns(self):
        ...
        # ✅ Good: Proper async with concurrent operations
        @app.get("/dashboard")
...
class CodeReviewPipeline:
    ...
    def generate_feedback(self, results: dict) -> dict:
        ...
        if len(feedback['critical_issues']) > 0:
            feedback['summary'] = f"❌ Found {len(feedback['critical_issues'])} critical issues that must be fixed"
        elif len(feedback['warnings']) > 0:
            feedback['summary'] = f"⚠️ Found {len(feedback['warnings'])} warnings to address"
        else:
            feedback['summary'] = "✅ Code looks good! No critical issues found"
...
**Status**: ❌ Changes Requested / ⚠️ Approved with Comments / ✅ Approved
...
| Security Score | 7/10 | >8 | ⚠️ |
| Performance Score | 89/100 | >90 | ⚠️ |
