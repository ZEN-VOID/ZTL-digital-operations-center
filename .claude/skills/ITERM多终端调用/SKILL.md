...
```python
# 场景1: 快速执行并获取结果
ITERM多终端调用.execute("git status")

# 场景2: 复杂命令，指定超时
ITERM多终端调用.execute("npm test", timeout=10)

# 场景3: 切换目录并执行
ITERM多终端调用.execute("cd /path/to/project && ls -la", timeout=5)
```
...
```python
# 场景4: 在新窗口执行
ITERM多终端调用.execute_in_new_window("npm run dev")

# 场景5: 在指定窗口执行
ITERM多终端调用.execute_in_window("git pull", window_index=2, timeout=5)

# 场景6: 查询所有窗口
windows = ITERM多终端调用.list_windows()
...
# 在窗口3监控日志
ITERM多终端调用.execute_in_window("tail -f app.log", window_index=3)
```
...
```python
# 检查Git状态
status = ITERM多终端调用.execute("git status --short", timeout=3)
print(f"Modified files: {status['output']}")

# 如果有修改，提交
if status['output']:
    ITERM多终端调用.execute('git add . && git commit -m "Auto commit"')
    ITERM多终端调用.execute("git push")
```
...
```python
# 运行测试并捕获结果
test_result = ITERM多终端调用.execute("npm test", timeout=30)

# 分析测试结果
if "PASS" in test_result['output']:
    print("✅ All tests passed")
    # 自动部署
    ITERM多终端调用.execute("npm run deploy")
else:
    print("❌ Tests failed")
    # 记录失败信息
...
# 收集系统信息
info = {
    "pwd": ITERM多终端调用.execute("pwd")['output'],
    "node_version": ITERM多终端调用.execute("node --version")['output'],
    "npm_version": ITERM多终端调用.execute("npm --version")['output'],
    "git_branch": ITERM多终端调用.execute("git branch --show-current")['output'],
    "disk_space": ITERM多终端调用.execute("df -h .")['output']
}
