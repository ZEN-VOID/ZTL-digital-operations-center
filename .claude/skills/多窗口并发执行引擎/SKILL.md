...
# 多窗口并发执行引擎 - 多线程执行器 (Blade Storm Multi-Threading Executor)

> "当单一上下文无法容纳时,多窗口并发执行引擎将任务分割为千万碎片,并行斩落"
...
# 方式2: 通过Bash启动Worker
Bash("bash .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/launch_workers.sh 美团数据分析")

# 方式3: 通过ResultAggregator监控
Bash("python3 .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/result_aggregator.py 美团数据分析 --monitor")
```
...
# Step 2: 启动Worker
Bash("bash .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/launch_workers.sh 火锅店开业海报")

# Step 3: 监控结果
Bash("python3 .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/result_aggregator.py 火锅店开业海报 --monitor")
```
...
Bash("bash .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/launch_workers.sh 用户认证系统开发")
...
# 手动触发汇总
python3 .claude/skills/多窗口并发执行引擎/multi-threading-executor/scripts/result_aggregator.py {项目名}
```
